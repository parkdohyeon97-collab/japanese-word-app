console.info("Japanese Word App v22.7 performance optimization loaded");
...
function getAllItems() {
  ...
  allItemsCache = [...baseWords, ...cloudItems];
  return allItemsCache;
}

function getCategoryItems(category) {
  if (categoryItemsCache.has(category)) {
    return categoryItemsCache.get(category);
  }
  ...
  studyTitle.textContent = studyMode === "graduated"
    ...
    : studyMode === "search"
      ...
      : studyMode === "random"
        ...
        : (() => {
          const categoryItemCount = getCategoryItems(currentCategory).length;
          const plan = getChapterPlans(currentCategory, categoryItemCount)
...
window.addEventListener("storage", event => {
  if (event.key === WRONG_COUNTS_KEY) wrongCountsCache = null;
  if (event.key === GRADUATED_ITEMS_KEY) graduatedMapCache = null;
});
